...
「提取 k 倍系数和」这种事情来个单位根反演就行了，但是 $k$ 不保证质数诶…… 没关系，一定能找到质数 $p = vk + 1$，它的原根 $g$ 满足 $g^{p - 1} \equiv 1 \pmod{p}$（模 $p$ 原根 $g$ 的定义是，$\varphi(p)$ 是 $g$ 模 $p$ 的阶），即 $g^{vk} \equiv 1 \pmod{p}$，那么 $g^v$ 是模 $p$ 意义下的 $k$ 次单位根，带 $k$ 个它的幂次进去算行列式即可。
...
### [$JOI-$开荒者](https://uoj.ac/problem/355)
